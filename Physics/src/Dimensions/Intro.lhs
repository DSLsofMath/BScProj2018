...
- A **unit** is a certain magnitude of something. One meter is for instance approximatly the length of your arm.

What's the relation between these? A *quantity* has a *dimension*. The number describing the distance between Stockholm and Gothenburg is of the type length. A *quantity* also has a *unit* that relates the number to a known definite distance. The unit of a quantity must describe the dimension of the quantity. It's not possible to describe a distance with joule. However, describing a distance is possible with both meters and inches. Those are two different units describing a quantity of the same dimension.

The dimension of a quantity is often implicitly understood given its unit. If I have a rope of 1 meter, one knows it's a length I'm talking about.
...
The outline of this chapter is to first introduce dimensions on *value-level*. Then we'll do dimensions on *type-level*. And finally we'll combine those results to create a data type for quantities.
